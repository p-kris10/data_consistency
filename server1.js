...
io.on('connection', (socket) => {
    ...
    console.log("Connected with Client ",count);
    ...
    socket.on("TOKEN_REQUEST",function(data){
        ...
        if(x !== 1)
        {
            ...
            console.log("Client "+ map.get(socket.id) + " Pushed to queue");
        ...
        else
        {
            ...
            console.log("Token given to Client",map.get(socket.id));
            io.to(socket.id).emit("GIVE_TOKEN");
            // console.log("Token=",token);
        }
    });
    socket.on("EXECUTE_CS_BOOK",function(data){
        var vacci = data.vacc;
        var q = parseInt(data.quantity,10);
        console.log("Client "+map.get(socket.id)+" trying to Book Vaccine");
        //CS
        // console.log(typeof vacci);
        if(vacci === "p")
        {
            vData.findById("616bf50df53a9a9651d49fe1",(error,data)=>{
                if(error)
                {
                    console.log(error)
                }
    ...
    socket.on("EXECUTE_CS_CANCEL",function(data){
        var vacci = data.vacc;
        var q = parseInt(data.quantity,10);
        console.log("Client "+map.get(socket.id)+" Cancelling Appointment");
        //CS
        // console.log(typeof vacci);
        if(vacci === "p")
        {
        ...
        io.to(socket.id).emit("cancelled");
    },5000);
        
        
    });
    socket.on("RETURN_TOKEN_ON_FAIL",function(data){
        emitter.emit("token recieved");
        console.log("Token returned by Client",map.get(socket.id));
        if(queue.length != 0)
        {
            const s = queue.shift();
